...
import operator
import time
#print "Importations completed."

def Hurdle1(instanceNumber):
    ...
    def overlapTest(x,y,r): #tests for overlaps
        ...
        for i in xrange(len(spheresX)):
            if sqrt((x-spheresX[i])**2+(y-spheresY[i])**2) < spheresR[i]+r: #if the sphere radii overlap
                return i
        return -1

    def drawMap(): #draws a top-down map.  mainly used for testing purposes
        #print "drawing..."
        winString = 'Hurdle 1 - Iteration ',iteration," of ", numIterations
    ...
    def depressionSize(m,R):
        ...
        return 2*R**2*glassDensity*g*(1-(sandDensity/glassDensity))/(9*sandViscosity) #pending more complex function
    ...
    def moveSphere(xpos,ypos,mass):
        ...
        if c != -1: #if the beetle has collided with a sphere
            ...
            vmag = (0*(sphereMass(c)-mass)+2*mass*beetleVelocity)/(sphereMass(c)+mass) #calculate final velocity of elastic collision
            ...
            while vmag > 0:
                ...
                #d = overlapTest(xiOld, xiOld, beetleSize)
                #if d != -1: #if the beetle has collided with a sphere
                    #moveSphere(xiOld, yiOld, sphereMass(c))
                #print "Depressing sand from sphere ", c, " at " ,xiOld, "," ,yiOld," with mass ",sphereMass(c)
            spheresX[c] = xiOld
            spheresY[c] = yiOld
    ...
    def writeSpheresDepth():
        ...
        spheresDepthPerIteration.append(spheresDepth)
    ...
    def writeSandData():
        ...
        filename = "All_Sphere_Heights_",instanceNumber,".dat"
        filehandle = open(filename, "w")
        print "Writing height data to file..."
    ...
    def returnFinalData():
        ...
        for i in xrange(numSpheres-1):
            returnString += "{",spheresR[i],",",spheresDepth[i],"}, "
        returnString += "{",spheresR[numSpheres-1],",",spheresDepth[numSpheres-1],"}"
        return returnString
        ...
        for i in xrange(numSpheres):
            for j in xrange(numIterations):
                if j != numIterations - 1 and i != numSpheres - 1:
                    returnString += "{",spheresR[i],",",spheresDepthPerIteration[j][i],",",j,"}, "
        returnString += "{",spheresR[numSpheres-1],",",spheresDepth[numIterations-1][numSpheres-1],"}"
        return returnString
    ...
    sphereBufferSize = 50 #closest distance one sphere can be from another.  prevents interactions between multiple spheres
    numSpheres = 20 #Number of spheres to generate
    iterationsPerSecond = 1 #number of iterations that equate to 1 second of time passing
    numIterations = 3 #Number of iterations to run the "level" through before recording final data.
    sandAdded = 1 #layers of sand that are added to the grid over each iteration.
    sandSize = 1 #size of sand grain in mm.  (!) Very small sandSize can overload memory.
    mathematicaExportResolution = 10 #takes every nth element for rows and columns of sandLevel() to keep the output manageable
    ...
    minSphereSize = sandSize/2 #minimum radius of sphere in mm
    maxSphereSize = 100 #maximum radius of sphere in mm
    ...
    for iteration in xrange(numIterations):
        print "Starting iteration ",iteration," of ", numIterations,"."
        ...
        while overlapTest(x,y,beetleSize) != -1: 
        ...
        while checkSum != 0:
            ...
            moveBeetle(beetleX, beetleY)
            depressSand(beetleX, beetleY, beetleSize, beetleMass)
            moveSphere(beetleX, beetleY, beetleMass)
            eat(beetleX, beetleY)
        ...
        print "Iteration ",iteration," of ", numIterations, " completed."
    ...
    instanceFinalData = finalData()
    print "Retunring final data over time..."
    instanceFinalDataOverTime = finalDataOverTime()

    finalReturn = [instanceFinalData,instanceFinalDataOverTime]
...
finalData = "{"
finalDataTime = "{"
for indexer in xrange(numSimulations):
    instanceData = Hurdle1(indexer)
    #append the strings to the Mathematica output
    finalData += instanceData[0]
    if indexer != numSimulations - 1:
        finalData += ", "
    else:
        finalData += "}"
    finalDataTime += instanceData[1]
    if indexer != numSimulations - 1:
        finalDataTime += ", "
    else:
        finalDataTime += "}"

filehandle = open("Final_Sphere_Heights_Composite.dat", "w")
print "Writing composite height data to file..."
filehandle.write(finalData)
filehandle.close()

filehandle = open("All_Sphere_Heights_Composite.dat", "w")
